      ...
      switch (event.httpMethod) {
        case 'GET':
          ...
          if (segments.length === 1) {
            ...
            const rows = await sheet.getRows(); // can pass in { limit, offset }
            const srows = rows.map(serializeRow);
            var foundRow = search(rowId, srows);
            if (foundRow) {
              delete foundRow.Email;
              return {
                statusCode: 200,
                body: JSON.stringify(foundRow) // just sends less data over the wire
              };
            } else {
              return {
                statusCode: 201,
